...
bool visible(
    ...
    if (var.arguments.size() != 1) {
        throw std::runtime_error("converted_gossip visibility expects exactly one target argument");
    }
    ...
    throw std::runtime_error("converted_gossip visibility does not know schema: " + var.schema);
}

const VisibilityModelRegistration registration("converted_gossip", visible);
