...
class ExternalFunction:
    ...
    def get1o(self,parent_state,agt_id):
        ...
        all_agent_last_call_index_dict = dict()
        all_agent_calling_history_dict = dict()
        max_value_length = 0
        for var_index,value in parent_state.items():
            if "called" in var_index:
                target_index = var_index.split(" ")[-1]
                value_list = value.split("-")
                if len(value_list) > max_value_length:
                    max_value_length = len(value_list)
                all_agent_last_call_index_dict.update({target_index:value_list})
        print(all_agent_last_call_index_dict)
        agt_believed_called_index = dict()
