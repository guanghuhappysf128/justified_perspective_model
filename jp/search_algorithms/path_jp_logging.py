...
class EnumEncoder(json.JSONEncoder):
    def default(self, o):
        if isinstance(o, Enum):
            return o.name   # <--- return key instead of value
        return super().default(o)

# the search name should be the same as python file name
class path_jp_logging(Search):
    ...
    def jp_logging(self,sgp_p_dict,key_variables,output_path):
        print("Logging validated plan actions:")
        print(sgp_p_dict)
        print(key_variables)
        # each state need to be filtered based on key_variables
        
        for key,local_p in sgp_p_dict.items():
            for state in local_p:
                for var_name in list(state.keys()):
                    if var_name not in key_variables:
                        del state[var_name]
        

        print(sgp_p_dict)
        # now save to file as json

        with open(output_path,"w") as f:
            json.dump(sgp_p_dict,f,indent=4,cls=EnumEncoder)
        print(f"JP logging saved to {output_path}")
